...
# ranked_entry_points_path = os.path.join(output_base_dir, 'ranked_entry_points'+fileName+'.csv')
# entrypoint_most_info_path = os.path.join(output_base_dir, 'entrypoint_most_info'+fileName+'.csv')
...
def analyze_nessus_file(driver, projectId, username, fileId):
    # try:
        ...
        ranked_entry_points_path = os.path.join(output_base_dir, 'ranked_entry_points'+file['file']+'.csv')
        entrypoint_most_info_path = os.path.join(output_base_dir, 'entrypoint_most_info'+file['file']+'.csv')
        ...
        project.updateProjectStatus(driver, projectId, username, 25, fileId)  # Update to 25%
        ...
        project.updateProjectStatus(driver, projectId, username, 40, fileId)  # Update to 40%
        ...
        df['viable_exploit'] = df['viable_exploit'].replace({'True': 1, 'False': 0})
        viable_exploit = df['viable_exploit'].astype(int)
        encoded_data = pd.concat([encoded_data, viable_exploit], axis=1)
        project.updateProjectStatus(driver, projectId, username, 55, fileId)  # Update to 55%
        ...
        project.updateProjectStatus(driver, projectId, username, 65, fileId)  # Update to 65%
        ...
        project.updateProjectStatus(driver, projectId, username, 80, fileId)  # Update to 80%

        # Save the result to a CSV file
        ranked_entry_points.to_csv(ranked_entry_points_path, index=False)
        project.updateProjectStatus(driver, projectId, username, 90, fileId)  # Update to 90%
        ...
        project.updateProjectStatus(driver, projectId, username, 'reports', fileId)  # Final update to 100%
